...
async function mergeMedia(rmqMessage: RmqEventMessage<MediaAdd>, mediaObject: MediaObject) {
  const redisClient = await redisConnection;
...
export async function MEDIA_PROGRESS(rmqMessage: RmqEventMessage<MediaAdd>) {
  const redisClient = await redisConnection;
  ...
  const isLastChunk: boolean = rmqMessage.data.isLast;
